    ...
    <>
      ...
      <div className="elative flex flex-wrap items-center justify-between px-6 bg-white md:px-16 lg:px-56 h-fit py-3  border-b-2 border-gray-100">
          ...
          <div className="w-full flex justify-start align-middle  m-only">
            {user ? (
                ...
                <Link to={`/GrantBiz/Profile`}>
                  {profile_avatar ? (
                    <img
                      src={profile_avatar}
                      alt="Avatar"
                      className="text-stone-400 h-16 w-16 active:text-primary"
                    />
                  ) : (
                    <IoPersonCircleOutline className="text-stone-400 h-16 w-full active:text-primary" />
                  )}
                </Link>
                <span className="flex items-center justify-center text-stone-500 px-2">
                  {Name}
                </span>
              </>
            ) : (
              <div className="flex justify-between align-middle w-full ">
                <Link
                  className="flex justify-start align-middle w-full "
                  to="/GrantBiz/SignIn"
                >
                ...
                <div className="flex w-full items-center justify-end">
                  <Link
                    className="md: btn rounded-full py-2 px-4 md:border-2 border border-primary"
                    to="/GrantBiz/SignIn"
                  >
        ...
        <img
          src={GrantBizLogo}
          alt="Logo"
          className="pc-only md:w-24 md:h-24 ml-0 "
        />
        <div className="flex flex-col px-3 w-full md:w-fit">
          {/* nav items PC only */}
          <div className="flex items-center justify-center ">
            <Link className="p-3 pc-only" to="/GrantBiz/">
              Home
            </Link>
            <Link className="p-3 pc-only" to={`/GrantBiz/Feed`}>
              Feed
            </Link>
            <Link className="p-3 pc-only" to="/GrantBiz/Order">
              Order
            </Link>
            <Link className="p-3 pc-only" to="/GrantBiz/Cart">
              Cart
            </Link>
            <Link className="p-3 pc-only" to={`/GrantBiz/MyShop/${email}`}>
              My Shop
            </Link>
            <Link className="p-3 pc-only" to={`/GrantBiz/Profile`}>
              My Profile
            </Link>
          </div>
        </div>
